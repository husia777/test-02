...
async def add_products(
    ...
    product_service: ProductService = Depends(Stub(product_service_provider)),
):
...
async def aggregate_product(
    ...
    if not await product_service.is_product_bound_to_shift_task(
        unique_product_code, batch_number
    ):
        raise HTTPException(
