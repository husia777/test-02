...
class ProductRepository(ProductRepositoryInterface):
    ...
    async def is_product_exists(self, unique_product_code: int):
        product = await self.session.execute(select(Product).where(Product.unique_product_code == unique_product_code))
    ...
    async def is_product_bound_to_shift_task(self, unique_product_code: int, batch_number: int):
        product = await self.session.execute(select(Product).where(Product.unique_product_code == unique_product_code).where(Product.batch_number == batch_number))
        return True if product.scalars().first() else False

    async def aggregate_product(self, unique_product_code: str, batch_number: int):
        query = await self.session.execute(select(Product).where(Product.unique_product_code == unique_product_code).where(Product.batch_number == batch_number))
